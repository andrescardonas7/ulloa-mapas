...
metadata:
  ...
  origin: "https://github.com/DenisSergeevitch/agents-best-practices (MIT v1.2.0)"
  hub: "RULES"
---
...
Vendored in this hub. Complements — does not replace — always-on security rules or Cursor-specific hardening:

| Need | Also use |
| --- | --- |
| Audit `.cursor/` hooks, rules, MCP (AgentShield) | `agent-harness-security` |
| Author or refine `SKILL.md` (routing, evals) | `skill-design-philosophy` |
| Trust-scan a new third-party skill | `skill-trust-audit` + `audit-skills-trust.mjs` |
| Product code auth, APIs, OWASP | `security-review` |
| GitHub Actions that invoke coding agents | `agentic-actions-auditor` |

Always-on rules under `.cursor/rules/10-security/` still apply on every task.
