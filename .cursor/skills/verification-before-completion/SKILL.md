...
- **Partial output** — Piping through `head`, stopping mid-log, or only reading the last line can hide failures above the fold; use full exit code + full relevant output.
- **Wrong command** — Running `lint` when the claim was about `test`, or `pnpm test` in the wrong package, gives false confidence; match the claim to the exact command.
- **Cached success** — A green run from an earlier session is not evidence for *this* change; re-run after the latest edits.
- **CI vs local** — "Works on my machine" while CI uses a different Node/OS/env matrix; cite the environment you actually verified.
