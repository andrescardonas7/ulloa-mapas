---
name: c-review
description: Use only when the Trail of Bits c-review Claude plugin is installed with its agents, prompts, and scripts available. Do not use the flat Cursor-synced copy by itself; use a normal security-review workflow instead.
allowed-tools: Agent AskUserQuestion SendMessage TaskCreate TaskUpdate TaskList TaskGet Grep Glob Read Write Bash
---

# C/C++ Security Review

## Cursor Flat-Sync Safety Gate

This skill depends on plugin-local assets outside `skills/c-review/`: `agents/`,
`prompts/`, and `scripts/`. A flat Cursor sync copies only this `SKILL.md`, so the
skill is not safe to execute unless the full Trail of Bits `c-review` plugin is
installed and `C_REVIEW_PLUGIN_ROOT` or `CLAUDE_PLUGIN_ROOT` resolves to that
plugin directory.

Before starting Phase 0, verify all of these exist:

- `${C_REVIEW_PLUGIN_ROOT}/agents/c-review-worker.md`
- `${C_REVIEW_PLUGIN_ROOT}/prompts/clusters/manifest.json`
- `${C_REVIEW_PLUGIN_ROOT}/scripts/build_run_plan.py`
- `${C_REVIEW_PLUGIN_ROOT}/scripts/generate_sarif.py`

If any check fails, stop immediately and tell the user this flat-synced copy is
disabled for safety. Do not improvise worker prompts, subagent names, or script
paths.
