...
# Uses direct invocation (no bash -c) to avoid command injection from prompt content
run_claude() {
    local prompt="$1"
    local timeout="${2:-60}"
    ...
    if [ -n "$allowed_tools" ]; then
        timeout "$timeout" claude -p "$prompt" --allowed-tools="$allowed_tools" > "$output_file" 2>&1
    else
