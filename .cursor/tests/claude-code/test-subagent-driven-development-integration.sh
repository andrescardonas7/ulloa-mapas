...
# --permission-mode bypassPermissions: Required for CI/automated runs; grants full tool access.
#   SECURITY: Do not run in untrusted environments; Claude can modify/delete files.
...
# Test 4: Spec compliance review before code quality review (workflow order)
echo "Test 4: Spec compliance before code quality..."
spec_line=$(grep -n -i "spec compliance\|spec review\|spec-compliance" "$SESSION_FILE" 2>/dev/null | head -1 | cut -d: -f1 || echo "")
quality_line=$(grep -n -i "code quality\|code review\|code-quality" "$SESSION_FILE" 2>/dev/null | head -1 | cut -d: -f1 || echo "")
if [ -n "$spec_line" ] && [ -n "$quality_line" ] && [ "$spec_line" -lt "$quality_line" ]; then
    echo "  [PASS] Spec compliance review precedes code quality review"
elif [ -z "$spec_line" ] || [ -z "$quality_line" ]; then
    echo "  [WARN] Could not detect spec/code-quality order in transcript (patterns not found)"
else
    echo "  [FAIL] Spec compliance should come before code quality (found at line $spec_line, quality at $quality_line)"
    FAILED=$((FAILED + 1))
fi
echo ""

# Test 5: Spec reviewer reads code independently (not just implementer's report)
echo "Test 5: Spec reviewer verifies code independently..."
if grep -qi "read.*code\|inspect.*code\|verify.*independently\|skeptical\|don't trust\|verify.*source" "$SESSION_FILE"; then
    echo "  [PASS] Spec reviewer reads/verifies code independently"
else
    echo "  [WARN] Could not verify spec reviewer reads code independently (heuristic not matched)"
fi
echo ""
