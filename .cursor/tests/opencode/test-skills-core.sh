...
# Test 0: Smoke test against actual skills-core.js when available (tests real lib, not inline copy)
LIB_PATH="$HOME/.config/opencode/superpowers/lib/skills-core.js"
if [ -f "$LIB_PATH" ]; then
    echo "Test 0: Testing actual skills-core.js library..."
    result=$(node -e "
        try {
            const lib = require('$LIB_PATH');
            const path = require('path');
            const testFile = '$TEST_HOME/test-skill/SKILL.md';
            require('fs').mkdirSync(path.dirname(testFile), { recursive: true });
            require('fs').writeFileSync(testFile, '---\nname: lib-test\ndescription: From real lib\n---\n# Content');
            const fm = lib.extractFrontmatter ? lib.extractFrontmatter(testFile) : null;
            if (fm && fm.name === 'lib-test') {
                console.log('LIB_PASS');
            } else {
                console.log('LIB_FAIL');
            }
        } catch (e) {
            console.log('LIB_SKIP');
        }
    " 2>&1)
    if echo "$result" | grep -q "LIB_PASS"; then
        echo "  [PASS] Actual skills-core.js loaded and extractFrontmatter works"
    elif echo "$result" | grep -q "LIB_SKIP"; then
        echo "  [SKIP] skills-core.js not loadable (module format or deps)"
    else
        echo "  [WARN] skills-core.js test inconclusive"
    fi
    echo ""
fi

# Test 1: Test extractFrontmatter function (inline contract test)
